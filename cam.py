...
while run:
    ...
    try:
        ...
        rectCon = af.rectContour(contours) # FILTER FOR RECTANGLE CONTOURS
        biggestPoints= af.getCornerPoints(rectCon[0]) # GET CORNER POINTS OF THE BIGGEST RECTANGLE
        gradePoints = af.getCornerPoints(rectCon[1]) # GET CORNER POINTS OF THE SECOND BIGGEST RECTANGLE
        ...
        if biggestPoints.size != 0 and gradePoints.size != 0:
            ...
            biggestPoints=af.reorder(biggestPoints) # REORDER FOR WARPING
            ...
            gradePoints = af.reorder(gradePoints) # REORDER FOR WARPING
            ...
            boxes = af.splitBoxes(imgThresh) # GET INDIVIDUAL BOXES
            countR=0
            countC=0
            ...
            # DISPLAYING ANSWERS
            af.showAnswers(imgWarpColored,myIndex,grading,ans) # DRAW DETECTED ANSWERS
            af.drawGrid(imgWarpColored) # DRAW GRID
            imgRawDrawings = np.zeros_like(imgWarpColored) # NEW BLANK IMAGE WITH WARP IMAGE SIZE
            af.showAnswers(imgRawDrawings, myIndex, grading, ans) # DRAW ON NEW IMAGE
